...
<aside class="w-64 h-screen bg-white border-r shadow-sm dark:bg-neutral-900 dark:border-neutral-800">
  ...
  <!-- Navigation -->
  <nav class="flex flex-col px-4 space-y-1 text-sm">
    <!-- Dashboard -->
    <x-responsive-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')">
      <div class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-gray-100 dark:hover:bg-neutral-800 transition-all">
    ...
    <x-responsive-nav-link :href="route('user.index')" :active="request()->routeIs('user.index')">
      <div class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-gray-100 dark:hover:bg-neutral-800 transition-all">
        ...
        <span class="text-gray-700 dark:text-white">Kelola Akun</span>
      </div>
    </x-responsive-nav-link>
    ...
    <x-responsive-nav-link :href="route('symptom.index')" :active="request()->routeIs('symptom.index')">
      <div class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-gray-100 dark:hover:bg-neutral-800 transition-all">
        ...
        <span class="text-gray-700 dark:text-white">Kelola Gejala</span>
      </div>
    </x-responsive-nav-link>
    ...
    <x-responsive-nav-link :href="route('disease.index')" :active="request()->routeIs('disease.index')">
      <div class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-gray-100 dark:hover:bg-neutral-800 transition-all">
        <svg class="w-5 h-5 text-gray-700 dark:text-white" fill="currentColor" viewBox="0 0 20 20">
        ...
        <span class="text-gray-700 dark:text-white">Kelola Penyakit</span>
      </div>
    </x-responsive-nav-link>
    ...
    <x-responsive-nav-link :href="route('knowledge_base.index')" :active="request()->routeIs('knowledge_base.index')">
      <div class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-gray-100 dark:hover:bg-neutral-800 transition-all">
        ...
        <span class="text-gray-700 dark:text-white">Basis Pengetahuan</span>
      </div>
    </x-responsive-nav-link>

    <!-- Solusi Penyakit -->
    <x-responsive-nav-link :href="route('solution.index')" :active="request()->routeIs('solution.index')">
      <div class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-gray-100 dark:hover:bg-neutral-800 transition-all">
        <svg class="w-5 h-5 text-gray-700 dark:text-white" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
        ...
        <span class="text-gray-700 dark:text-white">Kelola Solusi</span>
      </div>
    </x-responsive-nav-link>
    ...
    <x-responsive-nav-link :href="route('laporan.diagnosa')" :active="request()->routeIs('laporan.diagnosa')">
      <div class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-gray-100 dark:hover:bg-neutral-800 transition-all">
        ...
        <span class="text-gray-700 dark:text-white">Laporan Diagnosa</span>
      </div>
    </x-responsive-nav-link>
    ...
    <form method="POST" action="{{ route('logout') }}">
      ...
      <x-responsive-nav-link :href="route('logout')" onclick="event.preventDefault(); this.closest('form').submit();">
        <div class="flex items-center gap-3 px-3 py-2 rounded-lg text-red-600 hover:bg-red-100 dark:hover:bg-red-900 dark:text-red-400">
