<x-app-layout>
    ...
    <div class="min-h-screen flex items-center justify-center py-12">
        <div class="w-full max-w-2xl bg-white p-6 rounded-lg shadow-lg">
            <form method="post" action="{{ route('symptom.store') }}" enctype="multipart/form-data" class="space-y-6">
                @csrf

                <div class="max-w-xl">
                    <x-input-label for="kode_gejala" value="Kode Gejala" class="text-gray" />
                    <x-text-input id="kode_gejala" type="text" name="kode_gejala" class="mt-1 block w-full text-black" value="{{ old('kode_gejala') }}" required />
                    <x-input-error class="mt-2" :messages="$errors->get('kode_gejala')" />
                </div>

                <div class="max-w-xl">
                    <x-input-label for="nama_gejala" value="Nama Gejala" class="text-gray" />
                    <x-text-input id="nama_gejala" type="text" name="nama_gejala" class="mt-1 block w-full text-black" value="{{ old('nama_gejala') }}" required />
