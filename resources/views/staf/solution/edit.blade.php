<x-app-layout>
    ...
    <div class="min-h-screen flex items-center justify-center py-12">
        <div class="w-full max-w-2xl bg-white p-6 rounded-lg shadow-lg">
            <form method="post" action="{{ route('solution.update', $solution->id) }}" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')

                <div>
                    <x-input-label for="solusi" value="Solusi" />
                    <x-text-input id="solusi" type="text" name="solusi" class="mt-1 block w-full text-black" value="{{ old('solusi', $solution->solusi) }}" required />
                ...
                <div class="max-w-xl">
                    ...
                    <select id="idDisease" name="idDisease" class="mt-1 block w-full rounded text-black border-gray-300 focus:border-indigo-500 focus:ring-indigo-500">
                        ...
                        @foreach ($diseases as $disease)
                            <option value="{{ $disease->id }}" {{ $disease->id == $solution->idDisease ? 'selected' : '' }}>
